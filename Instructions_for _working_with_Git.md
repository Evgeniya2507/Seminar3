...
Мы переносимся в журнал изменений, где видим список коммитов в хронологическом порядке с комментариями к ним. У каждого коммита есть свой уникальный идентификационный номер, а также их автор, дата и время создания.
...
Ветвления в Git нужны для того, чтобы несколько членов команды, работающие над одним проектом, могли вести совместную работу и при этом не мешали друг другу. Главная ветка (master) является, своего рода, чистовиком, в который заливается вся новая, проверенная и согласованная руководителем проекта (или другим ответственным лицом) информация из других веток.
...
После того, как вся необходимая информация из веток сливается в главную ветку "master", можно эти ветки удалить за ненадобностью. Для этого используются команды
...
Когда в одних и тех же строках, но в разных ветках написана разная информация, при слиянии может возникнуть merge-конфликт. Git не может определить самостоятельно какая версия файла верна, поэтому он предлагают варианты по выбору версии.

Ответственный за слияние выбирает необходимую версию, либо оставляет обе и редактирует текст вручную.

## Визуализация веток

Чтобы увидеть дерево коммитов с ветвлениями используем команду

    git log --graph

Для просмотра всех когда-либо существующих веток с деревом коммитов вводим команду
...
<<<<<<< HEAD
Для работы с удаленными репозиториями мы используем сервис **GitHub**. Он позволяет интегрироваться с программой Git и настоить работу с нашими репозиториями удаленно.
=======
...
Чтобы скопировать с чужого аккаунта на GitHub уделенный репозиторий, можно:
...
1. Набрать в поисковой строке на GitHub "<имя аккаунта/название удаленного репозитория>, при условии, что эти параметры известны. Скопировать ссылку этого репозитория. Создать папку на своем компьютере. Открыть ее в VScode и в терминале набрать команду:
    ...
    Git скопирует тот удаленный репозиторий и на нашем компьютере появится полная копия того репозитория. Для дальнейшей работы с файлами нам нужно пройти в папку с уже инициализированным репозиторием, которую мы склонировали с сервиса. Набираем команду

        cd <foldername>

    Теперь в командной строке видно, что мы перескочили в другую папку. Проверяем состояние репозитория по *git status* и, если все впорядке, то продолжаем нашу работу над файлом.
...
>>>>>>> 6a5732379cf7dee95977afafd3187abe0a45376f
